...
create table public.ai_evidence_followup_requests (
  ...
  constraint ai_evidence_followup_conversation_fkey
  ...
  constraint ai_evidence_followup_question check (
    char_length(trim(question)) between 1 and 4000
    and position(chr(0) in question)=0
  ),
...
create trigger ai_evidence_followup_set_updated_at
before update on public.ai_evidence_followup_requests
...
create policy ai_evidence_followup_select_owner
on public.ai_evidence_followup_requests for select to authenticated
...
declare
  ...
  target public.ai_interpretation_requests%rowtype;
  normalized_question text:=trim(p_question);
  question_hash text;
...
begin
  ...
  if normalized_question is null or char_length(normalized_question)<1 or char_length(normalized_question)>4000 or position(chr(0) in normalized_question)>0 then
    raise exception 'follow-up question must be between 1 and 4000 characters';
  end if;

  select * into target
  from public.ai_interpretation_requests
  where id=p_interpretation_request_id and requested_by=caller_id;
  ...
  if user_active>=3 or org_active>=15 then
    raise sqlstate 'PGRST' using
      message=jsonb_build_object('code','CONCURRENCY_LIMIT','message','Too many active evidence follow-up requests.')::text,
      detail=jsonb_build_object('status',429)::text;
  end if;

  insert into public.ai_evidence_followup_requests(
    interpretation_request_id,conversation_id,organization_id,project_id,requested_by,
    question,question_sha256,evidence_schema_version,evidence_snapshot,evidence_sha256
  ) values (
    target.id,target.conversation_id,target.organization_id,target.project_id,caller_id,
    normalized_question,question_hash,target.evidence_schema_version,target.evidence_snapshot,target.evidence_sha256
...
returns uuid language sql security invoker set search_path='' as $$
  select app_private.request_ai_evidence_followup(interpretation_request_id,question);
$$;
...
declare
  q record;
  target public.ai_evidence_followup_requests%rowtype;
  target_id uuid;
  current_evidence_hash text;
...
begin
  ...
  begin target_id:=(q.message->>'followup_request_id')::uuid;
  exception when others then perform pgmq.delete('ai_evidence_followup',q.msg_id); return; end;
  ...
  if current_evidence_hash<>target.evidence_sha256 or current_question_hash<>target.question_sha256 then
    update public.ai_evidence_followup_requests
      set status='error',processing_error='Follow-up immutable input integrity check failed.',processing_finished_at=now(),updated_at=now()
      where id=target.id;
    perform pgmq.delete('ai_evidence_followup',q.msg_id);
    return;
  end if;

  update public.ai_evidence_followup_requests
    set status='answering',processing_attempts=processing_attempts+1,processing_started_at=coalesce(processing_started_at,now()),processing_error=null,updated_at=now()
...
language sql security invoker set search_path='' as $$
  select * from app_private.claim_ai_evidence_followup_request(visibility_seconds);
$$;
...
declare
  ...
  direct_key_count integer;
  point_key_count integer;
begin
  ...
  if top_key_count<>5 or not (p_answer ?& array['schema_version','status','direct_answer','supporting_points','limitations']) then
    raise exception 'AI follow-up answer fields are invalid';
  end if;
  ...
  if jsonb_typeof(p_request.evidence_snapshot->'facts')<>'array' or jsonb_array_length(p_request.evidence_snapshot->'facts')<1 then
    raise exception 'AI follow-up evidence snapshot is invalid';
  end if;
  ...
  if status_value='answered' then
    ...
    if jsonb_array_length(direct_answer->'evidence_ids')<>0 or jsonb_array_length(p_answer->'supporting_points')<>0 or jsonb_array_length(p_answer->'limitations')<1 then
      raise exception 'insufficient-evidence AI follow-up shape is invalid';
    end if;
  end if;
end;$$;
...
begin
  ...
  if trim(p_provider) !~ '^[a-z0-9_-]{2,64}$' or nullif(trim(p_model),'') is null or char_length(trim(p_model))>128 or nullif(trim(p_prompt_version),'') is null or char_length(trim(p_prompt_version))>128 or p_policy_version<>'ai-evidence-followup-policy-v1' then
    raise exception 'AI evidence follow-up provenance is invalid';
  end if;
  ...
  update public.ai_evidence_followup_requests
    set status='completed',provider=trim(p_provider),model=trim(p_model),prompt_version=trim(p_prompt_version),policy_version=p_policy_version,
        answer_schema_version='ai-evidence-answer-v1',answer=p_answer,answer_sha256=answer_hash,processing_finished_at=now(),processing_error=null,updated_at=now()
    where id=target.id;
...
returns void language sql security invoker set search_path='' as $$
  select app_private.finish_ai_evidence_followup_success(message_id,followup_request_id,provider,model,prompt_version,policy_version,answer);
$$;
...
declare
  target public.ai_evidence_followup_requests%rowtype;
  retry_id bigint;
  safe_error text:=left(coalesce(nullif(trim(p_processing_error),''),'AI evidence follow-up failed.'),2000);
begin
  ...
  perform pgmq.delete('ai_evidence_followup',p_message_id);
  return 'error';
end;$$;
...
returns text language sql security invoker set search_path='' as $$
  select app_private.finish_ai_evidence_followup_error(message_id,followup_request_id,processing_error,retryable,max_attempts);
$$;
...
begin
  ...
  perform app_private.append_audit_event(
    new.organization_id,new.project_id,new.requested_by,case when auth.uid() is null then 'service' else 'user' end,
    event_name,'ai_evidence_followup_request',new.id::text,event_outcome,
    jsonb_strip_nulls(jsonb_build_object(
      'status',new.status,'previous_status',case when tg_op='UPDATE' then old.status else null end,
      'interpretation_request_id',new.interpretation_request_id,'question_sha256',new.question_sha256,
      'evidence_sha256',new.evidence_sha256,'answer_sha256',new.answer_sha256,
      'provider',new.provider,'model',new.model,'prompt_version',new.prompt_version,'policy_version',new.policy_version
    ))
  );
  return new;
end;$$;
create trigger ai_evidence_followup_audit_events
after insert or update on public.ai_evidence_followup_requests
