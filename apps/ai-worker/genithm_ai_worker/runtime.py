...
from .providers import JsonHttpClient, OpenAICompatibleProvider, ProviderConfig, ProviderHttpError, ProviderRouter
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        primary = ProviderConfig(
            name=os.environ.get("GENITHM_AI_PRIMARY_PROVIDER", "qwen").strip().lower(),
        ...
        backup_enabled = os.environ.get("GENITHM_AI_BACKUP_ENABLED", "true").strip().lower() not in {"0", "false", "no", "off"}
        backup = None
        if backup_enabled:
            backup = ProviderConfig(
                name=os.environ.get("GENITHM_AI_BACKUP_PROVIDER", "deepseek").strip().lower(),
...
def build_provider_router(settings: Settings) -> ProviderRouter:
    ...
    backup = OpenAICompatibleProvider(settings.backup, timeout_seconds=settings.request_timeout_seconds) if settings.backup else None
    ...
    def plan(self, user_message: str, authorized_context: dict[str, Any]) -> dict[str, Any]:
        def operation(provider: Any) -> dict[str, Any]:
            ...
            try:
                ...
                raise ProviderHttpError(f"structured planner output was invalid: {exc}", retryable=False, fallback_allowed=True) from exc
...
def process_once(settings: Settings, rpc: SupabaseRpcClient, planner: RoutedPlannerClient, interpreter: RoutedInterpreterClient | None = None, followup: RoutedEvidenceFollowupClient | None = None) -> bool:
    if process_plan_once(settings, rpc, planner): return True
    if process_interpretation_once(settings, rpc, interpreter or RoutedInterpreterClient(build_provider_router(settings))): return True
...
def build_clients(settings: Settings) -> tuple[RoutedPlannerClient, RoutedInterpreterClient, RoutedEvidenceFollowupClient]:
    return RoutedPlannerClient(build_provider_router(settings)), RoutedInterpreterClient(build_provider_router(settings)), RoutedEvidenceFollowupClient(build_provider_router(settings))


def run_forever(settings: Settings) -> None:
    rpc = SupabaseRpcClient(settings); planner, interpreter, followup = build_clients(settings)
    while True:
...
def main() -> None:
    ...
    if args.once:
        rpc = SupabaseRpcClient(settings); planner, interpreter, followup = build_clients(settings); process_once(settings, rpc, planner, interpreter, followup); return
