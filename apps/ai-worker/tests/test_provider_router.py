...
def test_primary_provider_handles_request_without_backup():
    ...
    assert router.run(lambda provider: provider.structured_response()) == {"ok": True}
...
def test_transient_primary_failure_falls_back_to_backup():
    primary = FakeProvider("qwen", "qwen-test", [ProviderHttpError("temporary outage", retryable=True, fallback_allowed=True)])
    backup = FakeProvider("deepseek", "deepseek-test", [{"ok": True}])
    router = ProviderRouter(primary, backup)
    assert router.run(lambda provider: provider.structured_response()) == {"ok": True}
    assert primary.calls == 1
...
def test_non_fallback_failure_stops_at_primary():
    primary = FakeProvider("qwen", "qwen-test", [ProviderHttpError("authentication failed", retryable=False, fallback_allowed=False)])
    backup = FakeProvider("deepseek", "deepseek-test", [{"ok": True}])
    router = ProviderRouter(primary, backup)
...
def test_router_rejects_same_provider_as_primary_and_backup():
    with pytest.raises(ValueError, match="must be different"):
        ProviderRouter(FakeProvider("qwen", "qwen-a", [{}]), FakeProvider("qwen", "qwen-b", [{}]))
